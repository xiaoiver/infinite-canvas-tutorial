import { simplex_3d } from './chunks/simplex-3d';

/**
 * 与站点 `packages/site/docs/components/shaders/mesh-gradient.ts` 片元逻辑一致；
 * 不混入片内随机 noise，可交给独立后处理。
 *
 * 顶点需声明与片元相同的 `std140` UBO 块，WebGL2 下仅扫描 VS 以 `uniformBlockBinding`。
 */
...
void main() {
  ...
  outputColor = vec4(gradientColor, 1.0);
}
`;
