...
export class ExportSVG extends System {
  ...
  execute(): void {
    this.vectorScreenshotRequests.added.forEach(async (vectorScreenshotRequest) => {
...
export function drawLinesGrid($namespace: SVGElement, gridColor: string) {
...
export function drawDotsGrid($namespace: SVGElement, gridColor: string) {
  const $defs = createSVGElement('defs');
  $namespace.appendChild($defs);
