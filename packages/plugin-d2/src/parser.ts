...
export const parseD2ToSerializedNodes = async (definition: string) => {
  ...
  shapes.forEach((shape: Shape & Text) => {
    ...
    const node = {
      ...
      width,
      height,
      fills: [{ type: 'solid', value: colors[fill.toLowerCase()], opacity: 1 }],
    ...
    if (label) {
      ...
      const labelNode: TextSerializedNode = {
        ...
        fills: [{ type: 'solid', value: colors.neutrals[color.toLowerCase()], opacity: 1 }],
        textAlign: 'center',
        textBaseline: 'middle',
  ...
  connections.forEach((connection) => {
    ...
    if (srcLabel) {
      ...
      const labelNode: TextSerializedNode = {
        ...
        fontStyle: italic ? 'italic' : 'normal',
        fontWeight: bold ? 'bold' : 'normal',
        fills: [{ type: 'solid', value: colors.neutrals[color.toLowerCase()] ?? 'black', opacity: 1 }],
        stroke: 'white',
        strokeWidth: 4,
        zIndex: 0,
        edgeLabelPosition: 0,
