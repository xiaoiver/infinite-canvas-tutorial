...
export async function convertToFiles(api: ExtendedAPI, node: SerializedNode, files: (FileUIPart | MaskPart | ShapePart | File)[] = [], parent?: SerializedNode): Promise<(FileUIPart | MaskPart | ShapePart | File)[]> {
  if (node) {
    ...
    //     }
    //   } else {
        const canvas = await api.renderToCanvas({
          ...node,
          x: 0,
          y: 0,
        }, { width: node.width as number, height: node.height as number });
        files.push({
          // id: node.id,
          // type: 'data-shape',
          type: 'file',
    ...
    // await Promise.all(api.getChildren(node).map(child => convertToFiles(api, api.getNodeByEntity(child), files, node)).filter(Boolean));
  }
