 ...
 **/
export function sliceTensor(tensor: Tensor, idx: number) {
  const [bs, noMasks, width, height] = tensor.dims;
  const stride = width * height;
  const start = stride * idx,
    end = start + stride;

  // @ts-expect-error - cpuData is not a property of Tensor
...
export function sliceTensorMask(maskTensor: Tensor, maskIdx: number) {
