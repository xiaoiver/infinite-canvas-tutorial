...
class SAM2 {
  ...
  sessionEncoder: [InferenceSession, string] | null = null;
  sessionDecoder: [InferenceSession, string] | null = null;
  image_encoded: { high_res_feats_0: Tensor, high_res_feats_1: Tensor, image_embed: Tensor } | null = null;
  ...
  async getORTSession(model: ArrayBuffer) {
  ...
  async getEncoderSession() {
    if (!this.sessionEncoder)
      this.sessionEncoder = await this.getORTSession(this.bufferEncoder!) as [InferenceSession, string];
  ...
  async getDecoderSession() {
    if (!this.sessionDecoder)
      this.sessionDecoder = await this.getORTSession(this.bufferDecoder!) as [InferenceSession, string];

    return this.sessionDecoder;
  }

  async encodeImage(inputTensor: Tensor) {
  ...
  async decode(points: { x: number, y: number, label: number }[], maskTensor?: Tensor) {
...
const stats = {
  device: 'unknown' as string | null,
  downloadModelsTime: [] as number[],
  encodeImageTimes: [] as number[],
