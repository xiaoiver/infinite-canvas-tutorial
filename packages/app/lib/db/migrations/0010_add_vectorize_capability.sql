-- 添加 'vectorize' capability 到 user_capability_preferences 表的 CHECK 约束
-- 需要先删除旧的约束，然后添加新的约束
...
-- 2. 添加新的 CHECK 约束，包含 'vectorize'（如果约束不存在）
DO $$
BEGIN
    -- 检查约束是否已存在
    IF NOT EXISTS (
        ...
        CHECK (capability IN ('text', 'chat', 'image', 'image-layered', 'vectorize', 'audio', 'embedding', 'vision'));
    END IF;
END $$;
