...
### 顺序图 {#mermaid-sequence}

<MermaidSequence />

### [WIP] 状态图 {#mermaid-state}
...
还可以通过垂直于边方向的偏移量和内部间距，确保文字与边线保持物理距离。我们增加 `edgeLabelOffset` 属性，和 `edgeLabelPosition` 不同，使用绝对单位，定义在法线方向上的偏移量。
...
作为背景色的轻量级替代方案，在文字周围添加描边或阴影，增强与背景的对比度。在 tldraw 中通过 `showTextOutline` 选项开启，在文字后绘制细轮廓，即使重叠也能保持可读性。

在 [课程 16 - 文本描边] 中我们介绍过这种实现。
...
[课程 16 - 文本描边]: /zh/guide/lesson-016#text-stroke
