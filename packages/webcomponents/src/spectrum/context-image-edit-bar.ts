...
export class ContextImageEditBar extends LitElement {
  ...
  private async removeBackground() {
    ...
    // TODO: use nano-banana or recraft to remove background
    await new Promise((resolve) => setTimeout(resolve, 2000));

    this.api.runAtNextTick(() => {
      // Create a new image besides the original one
      const newImage: RectSerializedNode = {
        id: uuidv4(),
        type: 'rect',
        fill: 'white',
        lockAspectRatio: true,
        x: this.node.x + this.node.width + 100,
        y: this.node.y,
        width: this.node.width,
        height: this.node.height,
      };
      this.api.updateNode(newImage);
      this.api.selectNodes([newImage]);
      this.api.record();
